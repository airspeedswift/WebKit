// Only this file's own boundary types, not the WebCore_Private umbrella: importing
...
// hands out as a contiguous span. Output is a *finished* token:
// `CSSParserTokenBits`, which is `CSSParserToken`'s own storage, written
// through the one factory per token kind in CSSParserTokenBits.h — each
// construction site knows its token kind statically, so no dispatch tag has
// to cross the boundary.
//
// What crosses in place of a pointer is an *offset*: `CSSParserToken`
// already stores a view into the input, so the pointer slot can carry the
// offset until `resolveValuePointer` runs on the C++ side, and Swift never
// forms or dereferences a pointer. The offset's top bit says whether it
// indexes the input or the chunk's unescape buffer.
//
// Numbers are not converted here, so C++ keeps calling charactersToDouble
// and double rounding stays bit-identical; until it runs, the union carries
// the number's own range, because once a dimension's number and unit have
// been merged the number is not recoverable from the value. Values
// containing escapes are unescaped into a buffer this file owns and hands
// to C++ alongside each chunk of tokens, so nothing has to be re-tokenized
// in C++.
//
...
/// A finished token, on its way out to C++.
///
/// `bits` is `CSSParserToken`'s own storage, written through the factories in
/// `CSSParserTokenBits.h` — one per token kind — so a construction site that knows its kind
/// statically, as all 43 of them do, never has to encode it for the consumer to decode again.
///
/// The two offsets are the observer's, not the token's: `CSSParserTokenBits` has no room for
/// a token's extent and must not grow, since it is the object the whole CSS parser stores.
/// They leave this struct again immediately, into the observer record buffer, and only when
/// there is an observer. They are here rather than returned separately because 24 + 4 + 4 is
/// 32 bytes, matching the return convention's previous size.
struct EmittedToken {
...
struct CSSTokenizerSwift<Unit: CSSCodeUnit>: ~Copyable {
    ...
    //
    // The C++ reads every character through `StringImpl::operator[]`, which
    ...
    // a literal, rather than encoded for C++ to decode. `@inline(always)` on all of them is what
    // makes the factories fold: with the kind constant the switch inside each factory disappears
    // entirely, none of the six is emitted out of line, and the type constant is shared with the
    ...
    /// `convertToDimensionWithUnit(StringView)` post-pass C++ used to run on the finished
    /// token, since there is nothing in it this function does not already know.
    ///
    /// That post-pass does two things. It resolves the unit's text to a `CSSUnitType`, via
    /// `cssPrimitiveValueUnitFromTrie` (transcribed at the end of this file). And it decides
    /// the token's value: `mergeIfAdjacent` joins the number and the unit into one view when
    /// they are physically adjacent in the input and the number is shorter than sixteen
    /// characters, so `10px` has value `"10px"` and round-trips verbatim through custom
    /// property serialization, while `1\70x` — the same unit written with an escape, whose
    /// text is a pooled String rather than a range of the input — has value `"px"`.
    ///
    ...
    /// selects which comparison a DimensionToken gets on whether it is zero, and custom
    /// property serialization reserializes from `value()`. The adjacency test below is
    /// *computed* rather than assumed, even though it holds structurally for every input
    /// either scanner can produce — `consumeNumber` ends the number's range at the cursor and
    /// `consumeName`'s fast path starts there, and `nextCharsAreIdentifier` only peeks — because
    /// getting it wrong would not necessarily show up in a token-by-token comparison against
    /// the real tokenizer, and the compare is free.
...
// `Scanner::Swift` is selected, and they are the only entry points reachable from the
// shipping parser.
...
///
/// Every buffer here belongs to Swift, and each chunk crosses to C++ as three `Span`s —
/// the direction a buffer handover can be made safe in, since `SafeInteropWrappers`
/// transforms *parameters*. The receiver is a refcounted shared reference, which is how a
/// directly-named callee reaches C++ state without a pointer parameter. This function
/// contains exactly one `unsafe`: the input span, which has to arrive *from* C++ and so
/// cannot be fixed by any annotation.
///
/// Owning the buffers here, rather than having C++ own them and pass pointers in, means
/// the cursor and block stack never need a rewind-and-grow protocol when a buffer fills
/// up, and there is no fixed nesting cap.
///
...
private func tokenizeAll<Unit: CSSCodeUnit>(
    ...
    // tokenize `10px`, an 8,192x over-allocation on a 4-byte input.
    //
...
/// production path (rdar://186723514). `takeChunk` below shows the other direction is safe.
///
/// Two things are on trust: the count, and the lifetime. The lifetime is worth stating
/// precisely, because the initializer is `@lifetime(borrow span)` so there *is* a
/// dependency — but it is on the imported `std::span` value, a pointer and a count with no
/// relation to the `StringImpl` that owns the bytes, and `_cxxOverrideLifetime` inside the
/// initializer makes it vacuous. Nothing ties the borrow to the owner, which is not the same
/// as there being no dependency at all.
///
/// Both hold in fact: the span is `m_input.currentString()`, and
/// `CSSTokenizerInputStream::m_string` is a `RefPtr<StringImpl>` — a strong reference — that
/// outlives this call. That `RefPtr` is the whole lifetime argument and nothing asserts it;
/// narrowing it to a `StringView` or a raw span to save eight bytes would silently invalidate
/// this comment.
///
/// This span also *aliases* memory C++ still reaches, and C++ is re-entered inside the borrow
/// via `takeChunk`. That is sound only because nothing on that path writes it: `takeChunk`
/// touches `m_input` only through the `const` `rangeAt`, `StringImpl` character storage is
/// immutable and never relocated, and growing `m_stringPool` moves eight-byte `RefPtr`s rather
/// than character data. A future edit that preprocessed or case-folded the input in place from
/// inside `takeChunk` would break it, and the compiler would not object — the `Span` arrived
/// through `@_unsafeNonescapableResult`, so it has been told to stop reasoning about this memory.
@_expose(Cxx)
...
//
// In this file, and not in one of its own, because WebCore compiles Swift with
// `-incremental -enable-batch-mode` and not whole-module optimization: nothing crosses a Swift
// file boundary, so with the trie one file away `consumeNumericToken` could not call the
// specialized `UInt8` trie in the same binary. Per dimension token it instead went through a
// lazy protocol-conformance-witness-table accessor to the unspecialized generic trie — every
// `Span` access through a witness — then `CSSUnitTypeSwift.rawValue` out of line, twice. Moving
// it here is worth 10.7 points of the 8-bit real-corpus median.
//
// `-wmo` on WebCore's Swift step would let the trie live in its own file again; changing that
// build setting affects every WebCore Swift file at once, so it is out of scope here.
...
//
// `CSSParserToken::stringToUnitType` resolves a DimensionToken's unit text to
// a `CSSUnitType`. Doing it here means only the resolved `CSSUnitType` has to
// cross to C++, in the seven bits `CSSParserTokenBits::unit` reserves for it,
// rather than the unit's range.
//
// This is a transcription, not a redesign: the length dispatch, the
// per-index nesting and the order of the comparisons are the C++'s. The one
// structural change is what lets a single body serve both widths. The C++
// writes `switch (toASCIILower(data[i]))`, where `toASCIILower` is
// `character | (isASCIIUpper(character) << 5)` for char16_t (ASCIICType.h:189)
// and a 256-entry `asciiCaseFoldTable` lookup for Latin1Character
// (ASCIICType.h:199); that table (ASCIICType.cpp:30) is the identity outside
// 'A'-'Z', so both spellings fold 'A'-'Z' and nothing else — Latin-1
// uppercase 0xC0-0xDE is not folded, nor is any non-ASCII UTF-16 code unit.
// Here that becomes `trieSymbol`, which folds 'A'-'Z', passes the rest of
// ASCII through, and collapses everything >= 0x80 to a single sentinel.
// Collapsing is behaviour-preserving because every edge in the trie is
// 'a'-'z' or '_', so no code unit >= 0x80 can match one either before or
// after folding.
//
// Zero `unsafe` markers. Every read is a bounds-checked `Span` subscript.
// The C++ has an `ASSERT(data.data())` that is a no-op in Release, so a null
// span there is unchecked; here the empty case is just `count == 0`, which
// is a real check rather than an assertion.

/// Mirrors `WebCore::CSSUnitType` (`Source/WebCore/css/CSSUnits.h`).
///
/// A mirror rather than the imported enum, because of the module this file imports:
/// `WebCore_Private.CSSTokenizerSwiftTypes` is deliberately not the WebCore umbrella (see
/// CSSTokenizerSwiftTypes.h), and `CSSUnits.h` cannot join it — it has no `#include`s and
/// leans on its includer for `uint8_t`, `std::optional`, `ASCIILiteral` and `NODELETE`, so
/// it is not self-contained. It is also a Private header, so putting it in this module would
/// exclude it from the `Core` umbrella and take `CSSUnitType` away from every other Swift
/// file that imports WebCore_Private. Mirroring costs 70 lines and no build risk.
///
/// `@c` (SE-0495) makes the mirror checkable: it emits this into WebCoreSwift-Generated.h as
/// a `uint8_t`-backed C enum, so CSSTokenizer.cpp can `static_assert` each C++ enumerator
/// against `CSSUnitTypeSwift<Name>` by name, for all 70. That matters because `CSSUnitType`
/// interleaves the aliases `FirstViewportCSSUnitType = Vw` and `LastViewportCSSUnitType = Dvi`
/// into the enumerator list, and the enumerator after an alias continues from the alias, so
/// moving either one silently renumbers everything below it. The trie only returns 63 of the
/// 70, so a mis-transcribed `Calc`, `Percentage` or `Integer` would not be caught behaviourally.
/// The aliases are not cases here — Swift enums cannot carry duplicate raw values — and the
/// asserts cover them against `Vw` and `Dvi` directly.
///
...
/// every test that does not contain a quirky-em, and would fail on all 13 spellings of it.
...
/// (`CSSParserToken.cpp`); it has no counterpart here because this file already knows
...
// shipping WebCore.framework.
//
// The input arrives packed into words rather than as a `Span` because a Swift *callee*
// cannot take one: the `__counted_by` + `noescape` recipe that gives
// `CSSSwiftTokenSink::takeChunk` a safe `Span` parameter only works in the C++-callee
// direction (rdar://186723514). The alternatives — an `UnsafeRawBufferPointer` parameter, or
// `Span(_unsafeCxxSpan:)` on a `std::span` — are the `unsafe` constructs this file must not
// contain.
//
// So the caller passes up to 16 code units by value, little-endian within each word, and the
// entry point rebuilds them into an `InlineArray` on the stack: no allocation, no pointer, no
// `unsafe`, and the exact length the caller asked for via `Span.extracting`. 16 covers every
// unit this trie can match (the longest is 5 characters). The cap is a `precondition` rather
// than a silent clamp, so a truncated input cannot silently agree with a truncated oracle.

/// The most code units a validation call can carry. See the note above.
private let maximumPackedCodeUnits = 16

/// `cssPrimitiveValueUnitFromTrie` over Latin-1 text, for tests.
///
...
/// `cssPrimitiveValueUnitFromTrie` over UTF-16 text, for tests.
///
