...
// call is needed inside the tokenizer. Values containing escapes are
// unescaped here, into a buffer the caller provides alongside the token
// buffer, and the token's range then indexes that buffer instead of the
// input.
...
/// Latin-1.
///
/// TODO(unsafe): the `unsafe` uses at these two entry points are the boundary — the
/// input span coming in, and the output buffer and state out-parameter going back.
/// There is no safe way to receive a writable buffer or an out-parameter from C++:
/// WTF has `BorrowedBytes` for reading and no mutable equivalent.
@_expose(Cxx)
