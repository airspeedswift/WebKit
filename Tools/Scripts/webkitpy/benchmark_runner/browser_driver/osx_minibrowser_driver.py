...
class OSXMiniDriver(OSXBrowserDriver):
    ...
    #
    # No retry loop here: retrying would steal focus back on every attempt, making the
    # run impossible to interrupt from the terminal. Being behind another window is in
    # any case not harmful, because MiniBrowser disables window occlusion detection for
    # itself (see Tools/MiniBrowser/mac/WK2BrowserWindowController.m).
    @staticmethod
